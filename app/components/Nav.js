...
const Nav = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      scrolled ? 'bg-black/80 backdrop-blur-xl py-3 shadow-lg shadow-red-500/10' : 'bg-transparent py-6'
    }`}>
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center space-x-1 bg-white/5 backdrop-blur-sm rounded-full px-2 py-2 border border-white/10'>
            {navLinks.map((link) => (
              ...
              >
                ...
                <span className={`absolute inset-0 bg-gradient-to-r from-red-500/20 to-pink-500/20 rounded-full transition-all duration-300 ${
                  activeLink === link.href ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
                }`}></span>
                
                {/* Active indicator */}
                <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-red-400 transition-all duration-300 ${
                ...
                {/* Animated underline */}
                <span className='absolute bottom-0 left-0 w-0 h-[2px] bg-gradient-to-r from-red-400 to-pink-400 group-hover:w-full transition-all duration-500'></span>
              </Link>
            ))}
          </div>
          ...
          <div className='hidden md:block'>
            <button className='relative px-6 py-2.5 rounded-full font-medium text-sm overflow-hidden group'>
              <span className='absolute inset-0 bg-gradient-to-r from-red-500 to-pink-500 transition-transform duration-300 group-hover:scale-105'></span>
